...
class Graph(QWidget):
    ...
    def update_graph(self, frame):
        if self.signal_x[frame] < self.window_size:
            ...
            # Adjust window to show data in the current window range
    ...
    # Redraw the canvas after each update to reflect the changes
        self.canvas.draw()
        return self.signal_plot,


    def visualize_graph(self):
        # Store the animated_plot instance so that it doesn't get garbage collected
        self.animated_plot = animation.FuncAnimation(
            fig=self.fig, 
            func=self.update_graph, 
            frames=self.frames_num,
